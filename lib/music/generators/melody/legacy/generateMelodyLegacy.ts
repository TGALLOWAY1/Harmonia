/**
 * Legacy Melody Generation Engine (pre phrase-based refactor).
 *
 * Kept only for before/after comparison in scripts/analyzeMelodies.ts and
 * MELODY_ENGINE_ANALYSIS.md. Not used by the application. This is the
 * original note-by-note probabilistic walker: per chord it rolls a rhythm,
 * then picks each pitch via softmax sampling over locally scored candidates.
 * It has no motifs, no phrase plan, no contour, and no candidate selection.
 */
...
export function generateMelodyLegacy(options: MelodyGenerationOptions): Melody {
  const {
    scalePitchClasses,
    chords,
    style,
